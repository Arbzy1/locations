...
app.get("/api/overview", async (c) => {
  ...
  return c.json(await getOverview(db));
});

app.get("/api/days", async (c) => {
  const db = getDb(c.env);
  return c.json(await getDays(db));
});

app.get("/api/day/:date", async (c) => {
  const db = getDb(c.env);
  const result = await getDay(db, c.req.param("date"));
...
app.get("/api/analytics/monthly", async (c) => {
  const db = getDb(c.env);
  return c.json(await getAnalytics(db, "monthly"));
});

app.get("/api/analytics/yearly", async (c) => {
  const db = getDb(c.env);
  return c.json(await getAnalytics(db, "yearly"));
});

app.get("/api/analytics/day-trips", async (c) => {
  const db = getDb(c.env);
  return c.json(await getAnalytics(db, "day-trips"));
});

app.get("/api/analytics/corridors", async (c) => {
  const db = getDb(c.env);
  return c.json(await getAnalytics(db, "corridors"));
});

app.get("/api/analytics/facts", async (c) => {
  const db = getDb(c.env);
  return c.json(await getAnalytics(db, "facts"));
});

app.get("/api/route-progress", async (c) => {
  const db = getDb(c.env);
  return c.json(await getRouteProgress(db));
});
