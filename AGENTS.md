...
- Transactional email only (verify, reset, billing). Never put coordinates, place names, or day routes in email.
...
- Every new API route needs positive + unauthenticated + cross-tenant + demo-denied tests.
- Schema changes that add tenant tables need an RLS leak test.
- Stripe webhook tests cover invalid signature, replay, and entitlement grant/revoke.
- Never commit real Takeout JSON. Use tiny synthetic fixtures under `packages/db/src/timeline-import/fixtures/`.
<!-- /sync:cursor-rule -->
...
| `deploy:staging` / `deploy:prod` / `deploy:both` / `deploy:preview` | Wrangler `--env staging` / `--env production` / both / staging version upload |
...
- **Unit:** `npm run test:unit` (Vitest; no external services).
- **Integration:** `npm run test:integration` (`app.request()`, mocked Stripe/R2).
- **RLS:** `npm run test:rls` (real Postgres). Isolation is app-level **and** FORCE RLS.
- **E2E:** Playwright (`npm run test:e2e`) against `PLAYWRIGHT_BASE_URL` (default `http://127.0.0.1:8787`).
- Prioritize auth/tenant boundaries, billing webhooks, imports, and data-mutating routes.
- CI runs unit, integration, RLS, e2e, and `deps:audit`.
