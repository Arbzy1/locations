...
3. The consumer parses, replaces or merges the source, rebuilds tenant aggregates, updates job counts, deletes the R2 object, and emails `import_ready` or `import_failed` (counts only; no place names). `import_jobs.notified_at` prevents a queue retry from sending twice.
