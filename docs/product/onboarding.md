...
1. Create an account with Google (when enabled) or email + password, or use the closed-signup page if `DISABLE_SIGNUP=true`.
...
You can sign in with Google (when configured), a password, a one-time link, or a 6-digit code. Change email or password from Settings.
